...
from toontown.toonfest.TokenRewardGui import TokenRewardGui
from toontown.parties.PartyUtils import getPartyActivityIcon, getCenterPosFromGridSize

class DistributedToonfestActivity(DistributedObject.DistributedObject):
    ...
    def load(self):
        ...
        if self.wantRewardGui:
            ...
            self.rewardGui = TokenRewardGui(self.showRewardDoneEvent)
    ...
    def loadSign(self):
        ...
        textureNodePath = getPartyActivityIcon(self.activityIconsModel, actNameForSign)
    ...
    def showJellybeanReward(self, earnedAmount, jarAmount, message):
        if not self.isLocalToonInActivity() or base.localAvatar.doId in self.getToonIdsAsList():
            messenger.send('DistributedToonfestActivity-showJellybeanReward')
            base.cr.playGame.getPlace().fsm.request('activity')
