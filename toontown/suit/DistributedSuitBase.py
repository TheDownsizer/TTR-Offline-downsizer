...
class DistributedSuitBase(DistributedAvatar.DistributedAvatar, Suit.Suit, SuitBase.SuitBase):
    ...
    def beginSupaFlyMove(self, pos, moveIn, trackName):
        ...
        if moveIn:
            ...
            animTrack = Sequence(Func(self.pose, 'landing', 0), Wait(waitTime), ActorInterval(self, 'landing', duration=dur), Func(self.loop, 'walk'), Func(self.stickSuit))
            ...
            lerpPosTrack = Sequence(Wait(impactLength), Func(self.unstickSuit), LerpPosInterval(self, timeTillLanding, skyPos, startPos=pos))
