...
class ThrowTrackCalculator(TrackCalculatorBase):
    """Calculator for Throw gag track."""
    
    def calculate_attack(self, toon_id, attack):
        """Calculate throw gag attack."""
    ...
    def calculate_damage(self, toon_id, attack, target_list, atk_level):
        """Calculate throw gag attack damage."""
        ...
        for curr_target in range(len(target_list)):
            ...
            organic_bonus = self._check_gag_bonus(toon, THROW, atk_level)
            prop_bonus = self._check_prop_bonus(THROW)
            attack_damage = self._get_av_prop_damage(THROW, atk_level, toon.experience.getExp(THROW), organic_bonus, prop_bonus, self.battle_calculator.propAndOrganicBonusStack)
        ...
        if not valid_target_avail and self._prev_atk_track(toon_id) != THROW:
            self._clear_attack(toon_id)
    
    def create_target_list(self, attack_index, attack):
        """Create target list for throw attack."""
        # Use the base implementation for throw attacks
        return super().create_target_list(attack_index, attack)
    
    def is_unlure_attack(self, attack_index, attack):
        """Throw attacks unlure lured suits."""
        return True